...
# /api/session.json is the seam the interface reads. Seed it from the fixture
# so the site is never serving a 404 there; `atlas-publish` on the runner
...
# Everything except the payload gets a long cache; CloudFront is invalidated
...
aws s3 cp "$STAGE/api/session.json" "s3://$BUCKET/api/session.json" \
  --region "$REGION" --content-type application/json --cache-control "no-cache"
