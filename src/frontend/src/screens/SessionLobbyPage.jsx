...
function SessionLobbyPage({ access, groupSession, onStart }) {
  ...
  return (
    <main className="app-page">
      <section className="phone-screen session-screen" aria-label="Invite your date">
        ...
        <div className="session-content">
          ...
          {isHost ? <>
            <h1>Now get<br /><em>your date.</em></h1>
            ...
            <img className="session-qr" src={qrUrl} alt="QR code your date can scan to join you" />
            ...
            <div className="lobby-members" aria-live="polite">
              ...
              {groupSession.members?.map((member) => <span key={member.user.id}>{member.user.id === access.user.id ? 'You' : 'Your date'}</span>)}
            </div>
          </> : <>
