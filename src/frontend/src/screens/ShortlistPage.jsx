...
export default function ShortlistPage({ access, shareToken, onDone }) {
  ...
  useEffect(() => { loadShortlist(access, shareToken).then(setFilms) }, [access, shareToken])
  ...
  return <main className="app-page"><section className="phone-screen deck-screen">
    ...
    <article className="deck-card"><div className="deck-art" style={film.artwork_url ? { backgroundImage: `linear-gradient(0deg, rgba(23,19,16,.8), transparent), url(${film.artwork_url})` } : {}}><h2>{film.title}</h2></div><div className="deck-copy"><span>{film.year}</span><p>{film.description}</p><small>{film.note || 'Chosen for what it argues, not its genre'}</small></div></article>
