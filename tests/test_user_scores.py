...
def test_blind_pair_answers_are_traced_back_to_their_films(scored_atlas, monkeypatch):
    ...
    # A real deck is fifteen films sampled at random, which can deal a pair of
    # two odd-indexed films — and then no answer to that pair affirms axis 1,
    # so the leaning below lands on "balanced" and the test fails perhaps one
    # run in forty. What is under test is the tracing, not the sampling, so
    # deal a fixed deck: every pair holds exactly one even-indexed film, and
    # the orientation alternates so both branches of the choice below run.
